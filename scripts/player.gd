...
func attack():
	...
	timeout(duration, func (): attacking = false)
...
func _on_area_3d_body_exited(body):
	if body.has_method("take_damage") and body.is_in_group("enemies"):
		var is_crit = RandomNumberGenerator.new().randf_range(0, 1) <= crit_chance
		var multiplier = 2 if is_crit else 1
		body.take_damage(RandomNumberGenerator.new().randi_range(10, 15) * multiplier, is_crit, transform.basis.z)
